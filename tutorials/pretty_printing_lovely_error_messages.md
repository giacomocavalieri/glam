...
not writing a compiler after all - we're just going to do is pretty print them.
...
As you can see, every single space was broken into a newline. This is the
...
- the pretty printer runs into the group with all the documents, it tries to fit
  it on a single line, but the string `"one two three four"` is longer than 10
  characters
...
display a text whose words can wrap on newlines as needed:
...
### Gracefully handling newlines

There's only one small problem with this solution: we're not actually handling
newlines the way one would expect. You can immediately notice this if you add
...
Wait! The newline we had in the original text didn't get indented, this is a
surprise as I would have expected that to be indented as well.

The pretty printer behaved in a kind of surprising way and didn't add
any indentation before the newline... and rightfully so! The pretty printer is
doing the correct thing here, let me show you why.

When we build a document using `doc.from_string` the pretty printer will _never_
split that string, even if it where to exceed the line limit. If it tried to be
smart and split the raw strings provided with `doc.from_string`, it could end up
doing more harm than good.

> Imagine you're writing a pretty printer for a programming language and the
> pretty printer randomly inserted newlines producing an invalid program. That
...
So, where we run into a `"\n"` _we - human beings -_ expect to see a newline and
some indentation, but the pretty printer just sees a `doc.from_string("\n")` and
doens't perform any kind of inspection of the string's content.

Ok this is all nice and cool, but how can we actually add newlines that play
...
renders it as a newline. The only difference is that now the pretty printer
_knows_ that is is going on a newline and it can also add indentation!
...
> The bottom line is (I'm not sorry for this horrible pun): if we ever
> need to join documents together separating those on a new line, we should
...
error message that shouldn't be too hard.

### The error's header

The error's header is not too complex and doesn't require any careful handling.
We have to concatenate a couple of strings to get the desired look and turn
those into a fixed document:
...
Once again, we want to display the line where the error takes place as is.
The pretty printer should never try to split it or change it in any way;
imagine if the code in your error messages looked different from the actual code
you've written, that would be really confusing!
...
With the pointer we have to be careful with its size: it's going to be composed
of a single `"┬"` character and then a sequence of dashes to underline the whole
span. We're writing a function that can create such pointer given the length
it should cover:
...
> Notice how we use a `doc.line` here as well. Remember: every time you need to
> join documents by putting a newline in between those you should use a
> `doc.line`!
...
together! Out final function's type should turn an `Error` into a `Document`:
...
Not bad! It just needs some tweaking and it should be good to go:
...
pointer and not on a newline. In order to do so we can `doc.concat` the two
documents together.
...
We can achieve this by making a little change to the `line_doc` function:
...
- `doc.line` as a way to split documents on newlines, playing nicely with the
  pretty printer
